			...
			if (highightSyntax)
			{
				...
				switch (c.kind)
				{
					...
					case CXCursor_FirstDecl...CXCursor_LastDecl:
                        if (c.kind ==  CXCursor_ObjCImplementationDecl){
                            type = SCKObjCImplementationDecl;
                        }else if(c.kind ==  CXCursor_ObjCClassMethodDecl){
                             type = SCKObjCClassMethodDecl;
                        }
                        else {
				...
				if (nil != type)
				{
					[source addAttribute:kSCKTextSemanticType  value:type   range:range];
				}
            }
